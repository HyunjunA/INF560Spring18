...
my_csv = pd.read_csv('complete_data_1 month.csv')      
csvfile= open('complete_data_1 month.csv','r')
...
data=pd.read_csv('complete_data_1 month.csv',names=None)
originData=data.ix[:]
...
listOurRealBeforeSurgeryFeature.append('G30')
# re live
# data=data[listOurRealBeforeSurgeryFeature]
data.index = pd.RangeIndex(len(data.index))

#POST TX data removed
# del data['PX_STAT']
# del data['PSTATUS']
# del data['PTIME']
# del data['LOS']
...
# load data
# dataset = loadtxt('complete_data_1 month.csv', delimiter=",")
...
#fig = plt.gcf()
#fig.savefig('C:/Users/User/workspace/Hello_Test/src/result/test1.pdf')
...
for train, test in kfold.split(x_train_res, y_train_res):
    # model fiting with train set
    clf.fit(x_train_res[train], y_train_res[train])
    
    
totalPredicted=[]

for inTX in range(len(x_val)):


    #predict test dataset
    tepredicted=clf.predict([x_val[inTX]])
    totalPredicted.append(int(tepredicted[0]))
    
totalPredicted=np.around(totalPredicted)
